# -*- coding: utf-8 -*-
"""
Preenchedor SSA | v6 — Rateio por Verba
...
Fluxo:
  ...
  4. SSA       -> preenche via F8 (hotkey global), como na v5
...
class PreenchedorSSA:
    ...
    def __init__(self):
        self.app = ctk.CTk()
        self.app.title("Preenchedor SSA | v6 — Rateio por Verba")
        ...
        self.log("Aplicacao iniciada — v6 (rateio por verba).")
        if HAS_PYNPUT:
    ...
    def _construir(self):
        ...
        ctk.CTkLabel(esq, text="Preenchedor SSA — Rateio por Verba",
                     font=("Segoe UI", 24, "bold"), text_color="white").pack(anchor="w")
        ctk.CTkLabel(esq, text="Planilha  >  Verba  >  Rateio  >  SSA (F8)",
        ...
        self.lbl_passo = ctk.CTkLabel(dir_, text="Passo 1 de 4 — carregar planilha",
    ...
    def _ao_trocar_aba(self):
        atual = self.tabs.get()
        mapa = {self.T1: "Passo 1 de 4 — carregar planilha",
                self.T2: "Passo 2 de 4 — escolher a verba",
                self.T3: "Passo 3 de 4 — conferir o rateio",
                self.T4: "Passo 4 de 4 — preencher no SSA"}
    ...
    def carregar_perfil(self):
        try:
            ...
            if s:
                ...
                for ent, key, dflt in [(self.entry_delay, "delay", "0,15"),
    ...
    def aba_ssa(self, parent):
        ...
        self.titulo(c2, "Execucao por hotkey",
                    "1. Clique no primeiro campo do SSA.  2. Pressione F8 para preencher a linha atual.  "
                    "F6 pula, F9 volta.")
        ...
        if not self.global_on:
            try:
                ...
                if HAS_PYNPUT and pk is not None:
                    ...
                    self.global_on = True
                    self.btn_hotkey.configure(text="Desativar hotkey global",
                                              fg_color=self.NEUTRA, hover_color=self.NEUTRA_HV)
                    self.lbl_hotkey_mode.configure(text="Hotkey: Global (pynput)")
                    self.log("Hotkey global ativada (pynput): F8 preenche, F6 pula, F9 volta.")
                ...
                if HAS_KEYBOARD_PACKAGE and kb_global is not None:
                    ...
                    self.log("Hotkey global ativada (keyboard).")
                    self.set_status("hotkey global ativada", self.SUCESSO)
                    return
                detalhes = []
            ...
            self.app.bind("<F8>", self._on_f8_local)
            self._local_f8_bound = True
            self.btn_hotkey.configure(text="Ativar hotkey global", fg_color=self.AZUL,
                                      hover_color=self.AZUL_HV)
...
# cd "C:\_RPA"
# Remove-Item -Recurse -Force "C:\_RPA\SSA_v6" -ErrorAction SilentlyContinue
# & python.exe -m cx_Freeze PreenchedorSSA_v6.py `
#   --base-name Win32GUI `
#   --target-dir "C:\_RPA\SSA_v6" `
#   --target-name "PreenchedorSSA.exe" `
#   --packages customtkinter,darkdetect,pandas,openpyxl,pyautogui,pyperclip,pynput,PIL
# =========================================================
